        ...
        private Panel AddArrayList(int index, DataRow values)
        {
            ...
            i13.Click += (sender, e) =>
            {
                ...
                if (tidTmp != "0")
                {
                    ...
                    for (int i = 1; i < rowTeacher.Length + 1; i++)
                        teaNameArr[i] = rowTeacher[i - 1]["t_name"].ToString();
                    teaNameArr[0] = nowtea;
                }
                else
                {
                ...
                new AdminChangeInfo(teaNameArr, values["g_name"].ToString(), nowtea, newteaName =>
                {
                    ...
                    if (tidTmp != "0")
                    {
                        ...
                        int _resCount = Convert.ToInt32(oldtea["res_count"]);
                        for (int i = 1; i <= resCount; i++)
                            if (Convert.ToInt32(oldtea["g" + i]) == gId)
                            {
                                for (int x = i; x < resCount; x++) oldtea["g" + x] = oldtea["g" + (x + 1)];
                                oldtea["g" + resCount] = -1;
